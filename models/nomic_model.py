...
Nomic embeddings are optimized for semantic text search.
"""
...
def get_text_embedding(text: str) -> np.ndarray:
    ...
    try:
        ...
        if not text or text.strip() == "":
            print("⚠️  Empty text detected, returning zero vector")
        ...
        print(f"⚠️ Nomic Embedding Error: {e}")
...
def get_document_embedding(text: str) -> np.ndarray:
    ...
    try:
        ...
        print(f"⚠️ Nomic Document Embedding Error: {e}")
        return np.zeros(768, dtype=np.float32)
